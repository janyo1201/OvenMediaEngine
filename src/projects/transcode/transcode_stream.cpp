	...
	for(const auto &encode : encodes)
	{
		...
		for (const auto &video_profile : video_profiles)
		{
			if(video_profile.IsActive())
			{
				...
				{
					// Keep source dimension if -1 specified in width or height
					auto width = video_profile.GetWidth(), height = video_profile.GetHeight();
					if (width == -1)
					{
						width = (*input_video_track_iterator)->GetWidth();
					}
					if (height == -1)
					{
						height = (*input_video_track_iterator)->GetHeight();
					}
...
bool TranscodeStream::Push(std::unique_ptr<MediaPacket> packet)
{
	...
	if(media_type == common::MediaType::Video && media_codec == common::MediaCodecId::H264)
	{
		...
		_bsfv.convert_to(packet_for_encoding->GetData(), cts);
		packet_for_encoding->SetCts(cts);

		packet_for_encoding->_frag_hdr.reset();
